...
(function () {
    // var data_url = 'http://localhost:8000/monthly_index_website.csv';
    var data_url =  '//s3.amazonaws.com/org-emissionsindex/js/CO2-Chart/monthly_index_website.csv';
    var d3_me = Plotly.d3;
    var WIDTH_IN_PERCENT_OF_PARENT = 100,
        HEIGHT_IN_PERCENT_OF_PARENT = 100;//100;//95;
    var gd_me = document.getElementById('myDiv_month_co2');
    var gd3_me = d3_me.select("div#myDiv_month_co2")
        .style({
    ...
    var initial_width_me = window.innerWidth;
    ...
    d3_me.csv(data_url, function (error, data) {
        if (error) {
            console.log(error);
        } else {
            // var imperial_text_me = get(data, 'Imperial hovertext');
            var text_me = get(data, 'Emissions hovertext');
        ...
        if (initial_width_me < 500) {
            var layout1_me = frame1_me
            layout1_me.height = (initial_width_me - 32) / aspect_ratio;
            data_me[0].line.width = 1.5;


            Plotly.plot('myDiv_month_co2',
                data_me,
                layout1_me,
                plot_options_me)
        } else if (initial_width_me <= 767) {
            var layout2_me = frame2_me
            layout2_me.height = (initial_width_me - 32) / aspect_ratio
            ...
            Plotly.plot('myDiv_month_co2',
                ...
                layout2_me,
                plot_options_me)
        } else if (initial_width_me <= 1023) {
            var layout3_me = frame3_me
            layout3_me.height = (initial_width_me - 32) / aspect_ratio

            // left pad of 150 is reasonable at 1023, but too much at 768
            var percent_extra_pad_me = (1023 - initial_width_me) / (1023 - 768)
            layout3_me.margin.l = 150 - 40 * percent_extra_pad_me;
            ...
            Plotly.plot('myDiv_month_co2',
                ...
                layout3_me,
                plot_options_me)
        } else if (initial_width_me <= 1279) {
            var layout2_me = frame2_me
            layout2_me.height = (initial_width_me - 418) / aspect_ratio
            ...
            var layout2_me = frame2_me
            layout2_me.height = 752 / aspect_ratio
        ...
        frames_me = [frame1_me, frame2_me, frame3_me];
        ...
        window.addEventListener('resize', function () {
            ...
            var window_width_me = window.innerWidth;

            // Find the current y-axis title
            // If the title contains "kg", change the layout to SI
            // If not, change to imperial
            // var y_label_me = my_Div_me.layout.yaxis.title;
            // if (y_label_me.indexOf("kg") >= 0) {
            //     frames_me.forEach(si_layout_function);
            //     my_Div_me.layout.updatemenus.active = -1
            // } else {
            //     frames_me.forEach(imperial_layout_function);
            //     my_Div_me.layout.updatemenus.active = 0
            // };
            ...
            if (window_width_me < 500) {

                Plotly.relayout('myDiv_month_co2',
                    frame1_me);
            } else if (window_width_me <= 767) {
                frame2_me.margin.l = 110;

                // add updatemenus back
                // frame2_me.updatemenus = update_menu;
                Plotly.relayout('myDiv_month_co2',
                    frame2_me);
            } else if (window_width_me <= 1023) {
                // left pad of 150 is reasonable at 1023, but too much at 768
                var percent_extra_pad_me = (1023 - window_width_me) / (1023 - 768)
                frame3_me.margin.l = 150 - (40 * percent_extra_pad_me)

                // add updatemenus back
                // frame3_me.updatemenus = update_menu;
                Plotly.relayout('myDiv_month_co2',
                    frame3_me);
            } else if (window_width_me <= 1279) {
