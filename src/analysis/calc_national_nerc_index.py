...
def calc_national_gen_co2():
    ...
    national_co2 = (
        facility_co2.loc[:, "final co2 (kg)"]
        + self.extra_co2.loc[:, "elec fuel co2 (kg)"].groupby(["year", "month"]).sum()
    )
    ...
    national_gen = (
        ...
        .sum()
        .add(self.extra_gen_fuel["generation (mwh)"], fill_value=0)
...
def calc_national_gen():
    ...
    national_gen["elec fuel fossil co2 (kg)"] = (
        national_gen["elec fuel fossil co2 (kg)"] + self.extra_co2["elec fuel co2 (kg)"]
    )
